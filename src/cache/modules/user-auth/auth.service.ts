...
export class AuthService {
  ...
  async register(userRegisterDto: UserRegisterDto): Promise<any> {
    const userExists = await this._authRepository.findOne({ where: { username: userRegisterDto.phonenumber } });
    if (userExists) {
      return VndErrorType.USER_CONFLICT;
    }
    const token = "123456"
  ...
  async login(userLoginDto: UserLoginDto): Promise<any> {
    // Check exist
    const user: User = await this._authRepository.findOne({ where: { username: userLoginDto.username } });    
    if (!user) {
    ...
    if (!isMatch) {
      return VndErrorType.USER_UNAUTHORIZED;
    }
  ...
  async verifyotp(userRegisterDto: UserRegisterDto, otp:string): Promise<any> {
    ...
    if (cachotp !== otp){
      return VndErrorType.USER_OTP_NOT_MATCH;
    }
  ...
  async changepassword(userid:string, oldpassword: string, newpassword: string): Promise<any> {
    ...
    if (!isMatch) {
      return VndErrorType.USER_PASSWORD_NOT_CORRECT;
    }
      const salt = await genSalt(10);
      const password = await hash(newpassword, salt);
      await this._UserRepository.update(user.id, {password: password})
